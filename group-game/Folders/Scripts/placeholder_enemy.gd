...
var Heart_SCENE: PackedScene = preload("res://Folders/Scenes/healing_item.tscn")
var speed: float
var target: Node2D
var damage: int
var health: int
...
func _on_hurtbox_area_entered(area: Area2D) -> void:
	if area.is_in_group("Player") == true:
		...
		area.playsound(preload("res://Folders/Sounds/Sword Hit Flesh.mp3.mp3"))
		if health <= 0:
			var spawn_resource: consumable_resource = preload("res://Folders/Resources/Heart.tres")
			...
			area.playsound(preload("res://Folders/Sounds/universfield-slime-impact-352473.mp3"))
			queue_free()
...
func _damage(body: Node2D):
	...
	if player:
		player.playsound(preload("res://Folders/Sounds/Sword Hit Flesh.mp3.mp3"))
	if health <= 0:
		var spawn_resource: consumable_resource = preload("res://Folders/Resources/Heart.tres")
		var new_instance = Heart_SCENE.instantiate()
		new_instance.Attributes = spawn_resource #this part is where im having trouble
		new_instance.global_position = global_position
		get_parent().add_child(new_instance)
		player.playsound(preload("res://Folders/Sounds/universfield-slime-impact-352473.mp3"))
		queue_free()
